...
| loading | Boolean | `false` | 是否显示加载状态（同心圆环 spinner） |
| block | Boolean | `false` | 是否通栏（width: 100%），演示页与表单提交推荐开启 |
...
3. `loading` 为 `true` 时，左侧 `icon` 会被 **k-ring-spinner** 圆环替代（非字体 loader，避免旋转偏心）。
4. APP 端演示页建议加 `block` 保证通栏；icon+文字垂直对齐依赖 `line-height` 与 icon 槽同高。
