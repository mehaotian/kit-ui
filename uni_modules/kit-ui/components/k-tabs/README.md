...
2. 未设 `stretch` 时，若 Tab 总宽超出容器（徽标/长标题），组件会 **自动切换为 scrollable**（带 8px 滞后，避免临界宽度抖动）。
3. 维护时不要对内部 `forceScrollByOverflow` 加 reactive watch 触发重测，详见 `docs/k-tabs跨端实现说明.md` §3.4。
