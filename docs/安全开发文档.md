...
本文档详细阐述了 kit-ui 组件库在开发过程中需要遵循的安全规范和最佳实践，确保组件库的安全性和用户数据的保护。
...
const sanitizedContent = computed(() => {
  if (props.allowUnsafe) {
    console.warn('k-rich-text: allowUnsafe is enabled, content will not be sanitized');
